...
const submitForm = async () => {
  ...
  const getPuzzles = async (eloMin: number, eloMax: number, numberOfPuzzles: number) => {
    const response = await fetch(`https://api.thepawnsjourney.live/puzzles/rating/${eloMin}/${eloMax}?limit=${numberOfPuzzles}`)
    //const response = await fetch(`http://localhost:3000/puzzles/rating/${eloMin}/${eloMax}?limit=${numberOfPuzzles}`);
    const puzzleColection = await response.json();
