...
import type { BoardConfig, MoveEvent, MoveableColor, SquareKey } from 'vue3-chessboard'
...
const lichessAPIUrl = import.meta.env.LICHESS_API_URL || "https://lichess.org/api";
...
// Get puzzle data from lichess API
async function getPuzzleData(puzzleId: string) {
  const response = await fetch(`${lichessAPIUrl}/puzzle/${puzzleId}`);
  if (!response.ok) {
...
const playerColor = pgnMoves % 2 === 0 ? 'w' as TurnColor: 'b' as TurnColor;
const enemyColor: TurnColor = playerColor === 'w' ? 'b' : 'w' as TurnColor;
const fullPlayerColor = shortToLongColor(playerColor)

let boardAPI: BoardApi | undefined
const originalExpectedMoves = puzzleData.puzzle.solution
...
const pendingPlayerMoves  = getOddMoves(originalExpectedMoves)
const pendingEnemyMoves = getEvenMoves(originalExpectedMoves)
